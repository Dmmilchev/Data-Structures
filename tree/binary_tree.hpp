...
public:
    ...
    void add(const char* where, T data);
    bool containsHelper (T data, TreeNode<T>* node) const;
    bool contains(T data) const;
    // char* getPathTo(T data) const;
...
template <typename T>
void SearchTree<T>::add(const char* where, T data) {
    if (strlen(where) == 0) {
        assert(root == nullptr);
...
bool SearchTree<T>::containsHelper (T data, TreeNode<T>* node) const { 
    ...
    return containsHelper(data, node->left) || containsHelper(data, node->right);
}
...
// template <typename T>
// char* BinaryTree<T>::getPathTo(T data) const {
